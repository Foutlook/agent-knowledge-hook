...
- `.opencode/command/knowledge.record-fix.md`：用于人工纠正 PRD 理解、技术方案或 BUG 分析之后。AI 调用 `node agent-knowledge/bin/agent-knowledge.js record-fix --type <bug|prd|tech> --title "<纠错标题>"`；真实知识库分离时加上 `--knowledge-root <私有知识库根目录>`，生成 `inbox/` 记录，并在最终答复里提示记录路径。
